...
if __name__ == "__main__":
            ...
            for i in tqdm(range(25)):  # roll over testing period
                ...
                try:
                    ...
                    for col in ['cut_bins', 'med_train']:
                        cut_bins[col] = ', '.join(cut_bins[col])
    ...
    print(pd.concat(bins_list, axis=0))


    with engine.connect() as conn:  # record type of Y
        pd.concat(bins_list, axis=0).to_sql('results_bins_new', con=conn, index=False, if_exists='append')
    engine.dispose()