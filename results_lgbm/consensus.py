...
def calc_mae(yoy_merge):
    ...
    for p in set(yoy_merge['testing_period']):
        ...
        for i in set(yoy_merge['icb_code']):
            ...
            for c in set(yoy_merge['cv_number']):   # calculate detailed mae for each of 5 cv groups
                part_c = part_i.loc[part_i['cv_number']==c]

                try:
                    mae['{}_{}_{}'.format(p, i, c)] = part_mae(part_c)
                except:
    ...
    # calculate the relative performance of our model compared to consensus
    df['diff_ex_fwd'] = (df['lgbm_ex_fwd'] - df['ibes'])/df['lgbm_ex_fwd']
    df['diff_in_fwd'] = (df['lgbm_in_fwd'] - df['ibes'])/df['lgbm_in_fwd']
...
def act_lgbm_ibes(yoy_ibes_median, update):
    ...
    # pivot prediction for lgbm with fwd & l
